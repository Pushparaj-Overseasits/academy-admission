...
export default class Add extends React.Component {
  ...
  async createSemester() {
    ...
    try {
      const response = await fetch('http://localhost:8000/admin/semester/create', {
        method: 'POST',
  ...
  async loadSemester(id) {
    try {
      const response = await fetch(`http://localhost:8000/admin/semester/${id}`, {
        method: 'GET',
  ...
  render() {
    if (this.props.add === 'edit' && this.props.editId) {
      this.loadSemester(this.props.editId);
    }
    return (
      <>
        <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
          <div className="rounded-t bg-lightBlue-900  mb-0 px-6 py-6">
            <div className="text-center flex justify-between">
              <h6 className="text-blueGray-100 text-xl font-bold">Add/Edit Semester</h6>
              <button
